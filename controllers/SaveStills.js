...
const SaveStills = async (req, res, next) => {
  let connection;
  ...
  try {
    ...
    if (req.files.photos?.length > 0) {
      ...
      for (const photo of req.files.photos) {
        const photoName = await savePhoto(photo, 'stills'); // indicamos la coleccion a la que pertenece

        // insertamos referencia en bbdd
        await connection.query(
          `INSERT INTO stills (name)
            VALUES (?)`,
          [photoName]
        );
      }
      ...
      const photoName = await savePhoto(req.files.photos, 'stills');
